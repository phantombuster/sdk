...
var argv, baseDir, config, configPath, cson, datePrefix, defaultEndpoint, e, fs, i, len, loadConfig, needle, path, ref, ref1, script, updateScript, updateStoreInfo, validate, watch, watchOptions;
...
try {
  ...
  updateStoreInfo = function(updatedPath, fileType) {
    var account, i, jsonFile, len, linkedScriptCoffee, linkedScriptJs, localScript, mdFile, pbScript, ref, ref1, upload;
    ...
    upload = function(account, pbScript, localScript) {
      return fs.readFile(jsonFile, function(err, jsonText) {
        var e, jsonErr;
  ...
  updateScript = function(updatedPath) {
    var account, fileExt, i, len, localScript, pbScript, ref, upload;
  ...
  if ((ref = argv._) != null ? ref.length : void 0) {
    ...
    for (i = 0, len = ref1.length; i < len; i++) {
      script = ref1[i];
      if (!updateScript(fs.realpathSync(script))) {
    ...
    watch(baseDir, watchOptions, function(event, updatedPath) {
