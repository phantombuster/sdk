  ...
  updateStoreInfo = function(updatedPath, fileType) {
    ...
    upload = function(account, pbScript, localScript, jsonText, mdText) {
      ...
      return needle.post((account.endpoint || defaultEndpoint) + "/store-info/by-name/" + pbScript, payload, options, function(err, res) {
        var ref, ref1, ref2;
        if (err) {
          return console.log("" + (datePrefix()) + account.name + ": [API-STORE] " + localScript + ": " + (err.toString()));
        } else {
          if (((ref = res.body) != null ? ref.status : void 0) === 'success') {
            return console.log("" + (datePrefix()) + account.name + ": [API-STORE] " + localScript + " -> " + pbScript);
          } else {
            return console.log("" + (datePrefix()) + account.name + ": [API-STORE] " + localScript + ": " + (((ref1 = res.body) != null ? ref1.status : void 0) != null ? res.body.status : "Error") + ": " + (((ref2 = res.body) != null ? ref2.message : void 0) != null ? res.body.message : "HTTP " + res.statusCode));
          }
        }
      });
    };
      ...
      for (pbScript in ref) {
        ...
        if ((ref1 = path.join(baseDir, localScript)) === linkedScriptJs || ref1 === linkedScriptCoffee) {
          fs.readFile(jsonFile, function(err, jsonText) {
            if (err) {
              return console.log("" + (datePrefix()) + account.name + ": [API-STORE] " + jsonFile + ": " + (err.toString()));
            } else {
              return fs.readFile(mdFile, function(err, mdText) {
                if (err) {
                  return console.log("" + (datePrefix()) + account.name + ": [API-STORE] " + mdFile + ": " + (err.toString()));
                } else {
